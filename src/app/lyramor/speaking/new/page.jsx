...
export default function NewSpeaking() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Speaking entry added successfully!');
        router.push('/lyramor/speaking');
      } else {
        const data = await res.json();
        setError(data.error || 'Failed to create speaking entry.');
      }
    } catch {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Add Speaking Entry</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-800 border border-zinc-700/50 rounded-xl p-6 space-y-6">
        <div>
          ...
          <input id="event" name="event" type="text" placeholder="e.g., National Tech Summit 2024"
            value={formData.event} onChange={handleChange} required
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input id="organizer" name="organizer" type="text" placeholder="e.g., Kementerian Kominfo"
          ...
          <div>
            ...
            <input id="role" name="role" type="text" placeholder="e.g., Speaker, Moderator"
        ...
        <div>
          ...
          <TrixEditor inputId="speaking-new-description" value={formData.description}
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={submitting || imageUploading}
            ...
            <FiSave size={18} />{imageUploading ? 'Uploading image...' : submitting ? 'Saving...' : 'Save Speaking Entry'}
          </button>
        </div>
      </form>
    </div>
